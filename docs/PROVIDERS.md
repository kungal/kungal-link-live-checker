...
- **`errno` 映射**(本服务用的就是这张表):
  ...
  | `-9` | 分享需提取码(本层**无法判定死活**) | `unknown` / `passcode_required` |
  | `-21` | **分享不存在**(分享页标题"百度网盘-链接不存在",已删/已取消/已失效) | **`dead` / `share_not_found`** |
  | `140` | 链接格式错误("啊哦,链接出错了",`shareid:0`) | `unknown`(可能是畸形输入) |
  | 缺 `errno` 字段 / 其它 | —— | `unknown` / `unparseable_response`(打 WARN) |

- **实测来源**:对论坛库 48 条最早百度分享逐条打 `shorturlinfo`(2026-06-13,带 BAIDUID):`-21`×15 全部"链接不存在"=dead(逐条核对标题,无登录墙误判,故 `-21 → dead` 安全)。
- ⚠️ **`-9` 不能判 alive(2026-06-26 修正)**:一条**已知失效**的带密分享 `shorturlinfo` 先返回 `-9`、后返回 `-21`。说明 **`shorturlinfo` 的 `-9` 对"失效的带密分享"和"有效的带密分享"返回同一个码**,不带提取码这层根本分不出死活——所以 `-9` 必须 `unknown`,**不能 alive**(否则误报"未失效")。这与和彩云/123盘不同:那两家**有独立的"不存在"码/文案**把 dead 与"被锁但存在"分开,故它们的"锁住→alive"成立;百度没有,故百度回退 `unknown`。
- ⚠️ 其它陷阱:**`shareid`/`uk` 是从 shorturl 解码出来的,失效分享照样返回**,不能当存在信号;响应**缺 `errno` 字段**时也绝不 alive(防反爬空壳被当成 `errno:0`)。`shorturlinfo` 的 `-9`(需提取码)与下面 `/share/verify` 的 `-9`(提取码错误)**含义不同**,别混。

> 后续可恢复带密分享的 `alive`:调用方有提取码时走 `/share/verify` 拿权威码(`0`=对、`-7/-8`=删/过期…)再判;但这些码需先实测确认才可写死(尤其映射 `dead` 前)。当前先保守 `unknown`。
